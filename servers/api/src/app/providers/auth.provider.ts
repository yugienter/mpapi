...
export class AuthProvider {
  ...
  async setTokenToCookie(response, accessToken, refreshToken, isFastifyResponse = true) {
    ...
    const cookieOpt: CookieSerializeOptions = {
      ...
      path: '/',
      domain: (config.appEnv == 'production' || config.appEnv == 'stg' || config.appEnv == 'dev')
        ? this.configProvider.config.setCookieDomain : null,
   ...
   * @returns 検証結果がokかどうか
   */
  async checkServerAuthToken(token: string, payload: string): Promise<{
    isOk: boolean,
    calculated: string,
  }> {
