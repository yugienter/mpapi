...
export class UsersPersistence implements Coded {
  ...
  async getUsers(
    t: EntityManager,
    userIds: string[],
    opts: { maxNumberOfSignInHistories?: number } = UsersPersistence.defaultOptForUserSearch,
  ): Promise<ModifiedUser[]> {
    if (userIds.length == 0) {
      return [];
    }
    ...
    const users = await (async () => {
      let usersQuery = t.getRepository(User).createQueryBuilder('u').where('u.is_deleted = false');
      // 備考情報は明示的に取得しない限りは取ってこない（必要なものだけ絞り込む）
      usersQuery = usersQuery.addSelect(safeColumnsOfUserProfile.map((x) => `p.${x}`)).innerJoin('u.profile', 'p');
