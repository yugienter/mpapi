...
export class ValidationUtil {
  ...
  static comparisons = {
    '<':  function (startTime: Moment, endTime: Moment) { return startTime.isAfter(endTime) },
    '<=':  function (startTime: Moment, endTime: Moment) { return startTime.isSameOrAfter(endTime) },
    '>': function (startTime: Moment, endTime: Moment) { return startTime.isBefore(endTime) },
    '>=':  function (startTime: Moment, endTime: Moment) { return startTime.isSameOrBefore(endTime) },
  }
  private static get ajv() {
    ...
    const datetimeComparisonValidationFunct = function validate(
      schema,
      data,
      ...
      _.set(
        datetimeComparisonValidationFunct,
        'errors',
        [{keyword: 'datetimeComparison', message: 'invalid datetime.', params: {fromDatetimeMoment, toDatetimeMoment}}]
      )
    ...
    //   },
    const timeComparisonValidationFunct = function validate(
      schema,
      data,
      parentSchema: AnySchemaObject
    ) {
      ...
      const sourceTimeMoment = moment(parentSchema.timeComparison.targetHhMm, 'HH:mm')
        .set({year: defaultYear, month: defaultMonth, date: defaultDay})
      const targetTimeMoment = moment(data)
        .set({year: defaultYear, month: defaultMonth, date: defaultDay})
      _.set(
        timeComparisonValidationFunct,
        'errors',
    ...
    // },
    const timeRangeValidationFunct = function validate(
      schema,
      data,
      parentSchema: AnySchemaObject
    ) {
    ...
    if (!ValidationUtil._ajv) {
      ...
      ValidationUtil._ajv.addKeyword({
        ...
        type: 'string',
        $data: true,
        validate: datetimeComparisonValidationFunct,
        errors: true,
      })
      ValidationUtil._ajv.addKeyword({
        keyword: 'timeComparison',
        type: 'string',
        validate: timeComparisonValidationFunct,
        errors: true,
      })
      ValidationUtil._ajv.addKeyword({
        keyword: 'timeRange',
        type: 'string',
        $data: true,
        validate: timeRangeValidationFunct,
        errors: true,
      })
