...
export class FileUtil {
  ...
  static async getResizedBuffer(filepath: string, size: number) {
    ...
    if (meta.width >= meta.height) {
      return await tmpData.resize(size, Math.floor((size * meta.height) / meta.width)).toBuffer();
    } else {
      return await tmpData.resize(Math.floor((size * meta.width) / meta.height), size).toBuffer();
    }
  }
